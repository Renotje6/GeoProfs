...
export class SickReportsController {
	...
	@ApiResponse({ status: 201, description: 'Sick report created', type: CreateSickReportDto })
	@ApiResponse({ status: 400, description: 'User already has an active sick report' })
	...
	@ApiResponse({ status: 400, description: 'User does not have an active sick report' })
	@ApiResponse({ status: 403, description: 'User is not the owner of the sick report' })
	...
	@ApiResponse({
		status: 200,
		description: 'Returns all sick reports of employees in your department when logged in as manager,\nor returns all sick reports of the logged-in employee when logged in as employee',
		schema: {
			anyOf: [{ $ref: getSchemaPath(GetAllSickReportsDepartmentDto) }, { $ref: getSchemaPath(GetOwnSickReportsDto) }],
		},
	})
	...
	@ApiResponse({ status: 200, description: 'Returns the sick report', type: GetSickReportDto })
