...
describe("SickReportsService", () => {
	...
	beforeEach(async () => {
					...
					useValue: {
						...
						save: jest
							...
							.mockImplementation((report) =>
								Promise.resolve({ ...report, id: "report-123" }),
							),
		...
		sickReportRepo = module.get<Repository<SickReport>>(
			getRepositoryToken(SickReport),
		);
	...
	describe("findAllManager", () => {
		it("should return sick reports for manager department", async () => {
			...
			expect(managerQB.leftJoinAndSelect).toHaveBeenCalledWith(
				"manager.department",
				"department",
			);
			expect(result).toEqual([mockSickReport]);
	...
	describe("findMine", () => {
		it("should return employee sick reports", async () => {
			...
			(sickReportRepo.createQueryBuilder as jest.Mock).mockReturnValue(
				sickReportQB,
			);
	...
	describe("userHasActiveSickReport", () => {
		it("should return false if no active report", async () => {
			...
			expect(sickReportQB.andWhere).toHaveBeenCalledWith(
				"sickReport.endDate IS NULL",
			);
		});
	});
});
