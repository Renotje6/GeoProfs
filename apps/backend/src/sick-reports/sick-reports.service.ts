...
export class SickReportsService {
	...
	async findAllManager(user: User) {
		...
		return await this.sickReportRepository.find({
			where: {
				employee: In(employees.map((employee) => employee.id)),
				endDate: IsNull(),
			},
		});
	}
	...
	async findMine(user: User) {
		...
		return this.sickReportRepository
			.createQueryBuilder("sickReport")
			.where("sickReport.employeeId = :employeeId", { employeeId: employee.id })
			.getMany();
	}
	...
	async userHasActiveSickReport(user: User) {
		const employee = await this.employeeRepository
			...
			.where("employee.userId = :employeeId", { employeeId: user.id })
			.getOne();
		const sickReport = await this.sickReportRepository
			.createQueryBuilder("sickReport")
			.where("sickReport.employeeId = :employeeId", { employeeId: employee.id })
