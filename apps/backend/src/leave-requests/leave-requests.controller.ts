...
export class LeaveRequestsController {
	...
	async create(
		...
		try {
			const leaveRequest = await this.leaveRequestsService.create(
	...
	@ApiOperation({
		summary:
			"Get all leave requests. Managers can see all leave requests, employees can see their own",
	})
	@ApiResponse({
		status: 200,
	...
	async updateStatus(
		...
		try {
			const leaveRequest = await this.leaveRequestsService.updateStatus(
				id,
