...
export class LeaveRequestsService {
	...
	async create(createLeaveRequestDto: CreateLeaveRequestDto, user: User) {
		const employee = await this.employeeRepository.createQueryBuilder('employee').where('employee.userId = :userId', { userId: user.id }).getOne();
		...
		const timeDiff = createLeaveRequestDto.endDate.getTime() - createLeaveRequestDto.startDate.getTime();
	...
	async findAllManager(user: User) {
		const manager = await this.managerRepository.createQueryBuilder('manager').leftJoinAndSelect('manager.department', 'department').where('manager.userId = :userId', { userId: user.id }).getOne();
	...
	async remove(id: string, user: User) {
		...
		const employee = await this.employeeRepository.createQueryBuilder('employee').leftJoinAndSelect('employee.user', 'user').where('user.id = :userId', { userId: user.id }).getOne();
		...
		// Calculate days correctly
		const timeDiff = leaveRequest.endDate.getTime() - leaveRequest.startDate.getTime();
		const totalDays = Math.ceil(timeDiff / (1000 * 3600 * 24)) + 1;
